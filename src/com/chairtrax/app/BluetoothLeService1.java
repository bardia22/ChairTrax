...
import com.broadcom.util.GattAttributes;
...
public class BluetoothLeService1 extends Service {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (!loadServicesAndCharacteristics(gatt)) {
                	Log.e(TAG, "onServiceDiscovered error in discovery");
                	return;
                }
                setCharacteristicNotification(mSensorNotification, true);
    ...
    private void broadcastUpdate(final String action) {
        ...
        extras.putString(DEVICE_ADDRESS, mBluetoothDeviceAddress);
    ...
    private void broadcastUpdate(final String action,
        ...
        if (data != null && data.length > 0) {
        	float[] sensorData = SensorDataParser.processSensorData(data);
        	if (sensorData != null) {
        		extras.putFloatArray(EXTRA_DATA, sensorData);
        	}
        }
    ...
    public void setCharacteristicNotification(BluetoothGattCharacteristic characteristic,
        ...
        if (GattAttributes.SENSOR_NOTIFICATION_UUID.equals(characteristic.getUuid())) {
	        mSensorNotificationClientConfig.setValue(enabled ? 
    ...
    private boolean loadServicesAndCharacteristics(BluetoothGatt gatt) {
        // Get sensor service
        mSensorService = gatt.getService(GattAttributes.SENSOR_SERVICE_UUID);
        ...
        mSensorNotification = mSensorService.getCharacteristic(GattAttributes.SENSOR_NOTIFICATION_UUID);
        ...
        mSensorNotificationClientConfig = mSensorNotification.getDescriptor(GattAttributes.CLIENT_CONFIG_UUID);
        ...
        mBatteryService = gatt.getService(GattAttributes.BATTERY_SERVICE_UUID);
        if (mBatteryService != null) {
            mBatteryLevel = mBatteryService.getCharacteristic(GattAttributes.BATTERY_LEVEL_UUID);
        }
